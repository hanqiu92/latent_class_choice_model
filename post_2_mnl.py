...
def get_prob_c(alpha):
    global state
    K = alpha.shape[0]
    util_class = - np.square(np.sum(alpha.reshape((K,1,M)) * state,axis = 2))
...
def grad(w):
    ...
    grad_alpha = - 2 * np.sum( (prob_c - prob_x).reshape((K,N,1)) * np.sum(alpha.reshape((K,1,M)) * state,axis = 2).reshape((K,N,1)) * state, axis = 1)
...
def post_2_mnl(K = 1):
    weight = np.random.rand(M * K + m * K) - 0.5
    loss = L(weight)
    print 'initial: ',loss
    ...
    pickle.dump( {'alpha':alpha,'beta':beta}, open( "result/quebec_final_weight_post_2_mnl_"+str(K)+".p", "wb" ) )
